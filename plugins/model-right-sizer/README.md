...
- [`skills/model-right-sizer-chief-of-staff-drill/SKILL.md`](skills/model-right-sizer-chief-of-staff-drill/SKILL.md) — a companion skill that dogfoods the chief-of-staff status ledger: it gets a real blueprint, actually dispatches its `work_routing_map[]` units as live sub-agents, updates each row's `status` as a real orchestrator would, answers a mid-flight "where am I at" purely from the ledger, and scores the result against ground truth. Behavioral, not structural — `tests/test_validate_blueprint.py` already proves the schema shape.
...
Once a blueprint's `work_routing_map[]` is dispatched, the session driving that dispatch becomes the **chief-of-staff thread** for it: the one place that knows what went out, to which tier, and where each stands — the same fix that keeps a dozen loose AI conversations from becoming unmanageable, applied to a routing map's dispatched units. Each row carries a `status` (`not_started` → `dispatched` → `in_progress` → `done`/`blocked`) and a `status_updated_at` timestamp — schema-required, and always a real clock read, never guessed, once `status` leaves `not_started` — that the invoking session updates together as work proceeds, so a "where am I at" question gets one consolidated, freshness-checked answer instead of a tour through scattered sub-agent threads. See the "Chief of staff" section in `agents/model-right-sizer.md`.
