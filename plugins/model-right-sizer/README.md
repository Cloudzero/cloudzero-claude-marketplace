...
- [`skills/model-right-sizer-audit/SKILL.md`](skills/model-right-sizer-audit/SKILL.md) — a companion skill that retroactively audits every real model call already shipped in a target repo — one dry-run per decomposed call, via `model-right-sizer-dryrun` — and commits a single schema-conformant blueprint back to that repo through a PR. See its own "Action scope" note: unlike the other three skills, it writes one file and opens a PR in the *target* repo, not just this one.
...
The lineup it reasons over does not stop at the cheapest model with an invoice behind it. It carries a **local / open-weight row** as well (an on-device or self-hosted model on hardware the operator already owns, written `local:<model-id>` in a pick), deliberately narrow: mechanical, bulk-text, high-volume work off the critical path, unverified by default, and behind a three-step routing gate (deny, compound, propose) that fails toward the cloud. Because that tier has no vendor price page, the agent is required to price it rather than book it at $0: [`eval/local_tier.py`](eval/local_tier.py) implements the amortized device-plus-power basis, the expected-rework term, and the break-even against a hosted tier, and `scripts/validate_blueprint.py` rejects a local pick that never states what it costs. See "The tier below the cheapest invoiced model" in `agents/model-right-sizer.md`.
...
That installs the agent (`agents/model-right-sizer.md`) and all four companion skills (`skills/model-right-sizer-install/`, `skills/model-right-sizer-dryrun/`, `skills/model-right-sizer-audit/`, `skills/model-right-sizer-schema/`) together. Adding the marketplace also makes the [`cost-analyst`](../cost-analyst/) plugin available (`/plugin install cost-analyst@cloudzero`). To try it before installing, or to iterate on a local checkout, load it directly for a session instead:
...
and agent-schema-prescription outputs, and four companion skills — no
