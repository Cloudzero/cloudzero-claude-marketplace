...
## Chief of staff: what the invoking session does with `work_routing_map[]`

Your own job stops at the blueprint and the usage report — you don't dispatch anything and you hold no state between spawns. But the artifact you hand off, `work_routing_map[]`, names every unit of work the invoking session is about to scatter across sub-agents, models, or long-running threads. The moment it does that scattering, it inherits a role worth naming explicitly instead of leaving implicit: it becomes the **chief-of-staff thread** for this blueprint's dispatched work — the one place that knows what went out, to which tier, and where each stands, so a status question never has to be answered by re-opening every dispatched thread individually. That's the same failure mode a person hits juggling a dozen loose AI chats with no shared thread tracking them — the fix generalizes to a routing map's dispatched units just as well as to a person's conversations.

Concretely:
- Every `work_routing_map[]` row carries a `status` (`not_started` / `dispatched` / `in_progress` / `done` / `blocked`), always emitted `not_started` at blueprint time because nothing has shipped yet. The invoking session updates it in place as dispatch proceeds — the blueprint JSON doubles as a live **status ledger**, not just a one-time plan that goes stale the moment real dispatch starts.
- On a status request ("where am I at", "give me an update"), the invoking session's job is to check every row it's responsible for and return **one consolidated report** — not to make the requester re-derive it from N scattered sub-agent threads. This is the same instinct as Pass B's recommended-vs-actual reconciliation, pulled forward from "after the work closes" to "at any point someone asks."
- This is a role for **whoever invokes you**, not a new pass you run. Don't have Pass A or Pass B try to poll live dispatch state — you don't have it, and fabricating a status you can't observe would be worse than naming the gap. Name the duty here; let the invoking session actually carry it.
...
5. **`work_routing_map[]`** — the "who produces what" translation. Whoever reasons the *approach* hands it to you to translate into execution units, each assigned to the tier that should produce it, by the same difficulty→tier logic: one entry per build unit carrying `id, build_unit, derived_from_stage_id (nullable), pick, budget, handoff_schema_ref, status, rationale`. Small mechanical function → cheapest tier; bounded well-specified feature → mid tier; tricky/high-blast-radius or cross-cutting logic → top tier; large long-horizon refactor → the largest-context/longest-horizon tier available. Each unit carries an explicit token/thinking **budget** and a **handoff_schema_ref** so it both self-truncates and knows exactly what shape to hand the next unit. Every row's `status` starts `not_started` — nothing has been dispatched yet at blueprint time — and stays the invoking session's to update as dispatch proceeds; see "Chief of staff" below for why that matters. Leave the array empty only if no build units are decided yet (e.g. a dry run against a bare intent).
