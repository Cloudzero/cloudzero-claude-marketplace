...
### The tier below the cheapest invoiced model: local / open-weight inference

The lineup you fetch has a floor: the cheapest model somebody bills you for.
That is not the floor of the ladder. An open-weight model running on hardware
the operator already owns (an on-device runtime such as MLX or llama.cpp on a
developer machine, a self-hosted server on their own GPU) is one more row, and
it never shows up on a fetched price sheet because no vendor publishes a page
for it. It is the same shape of fork as the deterministic query layer above: a
tier below every hosted tier, not a new plane. In Pass A it is an ordinary
pick, written `local:<model-id>`.

It is a narrow row, and naming the narrowness is part of right-sizing it:

- **Where it earns the row:** mechanical, bulk-text, high-volume work off the critical path. Condensing text that already exists, recall-tuned pre-filtering where a stronger reader still sees every hit, quote-backed extraction, navigation aids over a large file. Marginal token cost is near zero, so volume that is uneconomic on any invoiced tier becomes free to run.
- **Where it does not:** any output that is a claim about state, a verification, a judgment, a ranking, or a sentence somebody outside the team will read. Every local result is unverified by default: input for a stronger model to check, or a signpost for a human, never a fact on its own. A small open-weight model is a fluent source of confident wrong answers, which is a different failure mode from a hosted tier being merely weaker.
- **A script beats it wherever a script fits.** Principle 5, one tier further down: deterministic code with a narrow read window costs less than any model and is right by construction. Route local only when the work is genuinely fuzzy AND high-volume AND off the critical path.
- **Capability, not price, is the binding constraint here,** which puts the Memory Wall directly under this row: past a task's model floor, more tokens on an undersized open-weight model buy nothing. A local pick is a capability claim in a way a hosted down-pin is not.
- **Throughput is the latency story, and it is opt-in.** One measured sample, illustrative, verify on your own hardware: a 4B 4-bit model on an M2 Pro laptop ran roughly 430 tok/s prompt and 60 tok/s generation at a 3K-token prompt, about 3.4GB peak. That is a background or scheduled job, never something an interactive path blocks on, and it makes an `agentic` local pick `measurement-required` under the same loop-class rule that governs a hosted down-pin. The tier also only exists where somebody owns the hardware and turned it on, so a stage routed local must behave identically when it is absent: fall back to the hosted pick, never make local the only path.
- **Residency is a second, non-cost reason to pick it.** Local inference never leaves the machine. For a stage handling contract terms, margin, or regulated data, local can be the safer route rather than merely the cheaper one, and that argument holds even where the token saving is negligible.

**A local run has no invoice, which is not the same as being free.** Booking one at $0 is the most expensive mistake available in this tier because it is unfalsifiable: every stage moved local shows unbounded ROI, and any calibration history learns from a saving that was really spend shifted onto hardware somebody already bought. Price it, and say which basis you used:

`P_local = (device_cost_per_hour + power_cost_per_hour) / tokens_per_hour` dollars per token (the figures below are that same number expressed per 1M), plus expected rework `p_wrong * wage_per_hour * rework_hours / tokens_per_run`.

That is Eq. 2 read literally rather than a new model: routing local does not delete a term of `TC = P_k·K + P_m·M + w·L`, it moves spend out of `P_m·M` (tokens somebody invoices) into `P_k·K` (capital already held), and into `w·L` when the small model is wrong. The basis choice is most of the answer and it spans an order of magnitude: charge a $3,000 machine over a 10,000-hour life plus 40W at $0.20/kWh, and 60 tok/s of generation costs about $1.43 per 1M tokens, the same order as the cheapest hosted tier. Count power alone, on the argument that the machine was bought anyway and would otherwise idle, and the same run costs about $0.04 per 1M. Both are defensible, a silent zero is not. Carry the basis on the price-sheet entry (`cost_basis: "amortized_local"`, with the derivation in `cost_basis_note`), never a zero rate. [`../eval/local_tier.py`](../eval/local_tier.py) implements both formulas and the break-even against a hosted tier.

### The routing gate in front of a local pick, and why it fails toward the cloud

A local pick is only safe behind a gate, evaluated in this order against the *instruction*, never against the data it will process (a document that happens to contain the word "overdue" must not force every digest of that document up a tier):

1. **Deny.** The instruction asks for a claim about state, a verification, an outward-facing artifact, a high-stakes domain, a judgment, or a ranking. Hosted tier, no appeal. This runs before any task match, so naming a safe task cannot smuggle a claim-shaped request down.
2. **Compound.** The instruction asks for more than one thing. A local task does exactly one narrow thing, so a two-clause request is definitionally not a match. This is the load-bearing gate: on one adversarial pass over a real implementation, 9 of 10 claim-shaped requests leaked past a keyword deny-list, every one of them a safe first clause the matcher hit on with the real ask in clause two ("digest X and tell me whether Y"). Blocklisting paraphrases cannot be finished; instruction shape can.
3. **Propose plus available.** Exactly one registered local task matches and the local runtime is actually present. Anything unmatched, ambiguous, or unavailable goes hosted. The gate may only ever propose, and silence means hosted.

Routing wrongly to a hosted tier costs tokens. Routing wrongly to local puts a confident wrong answer in front of somebody. Two rules follow from that asymmetry, both learned the expensive way on a real implementation: **give every local task a validator that can fail** (a shape-only check accepted 4 output lines for 6 input items, and the item-count invariant that caught it also pointed at the real fix, chunking by item), and **never ask a local model for something you can parse** (asked directly for checkbox state, a 4B model reported all 37 items in a file as unchecked, including items whose own text said CLOSED). Give it the fuzzy part only.
...
5. **`work_routing_map[]`** — the "who produces what" translation. Whoever reasons the *approach* hands it to you to translate into execution units, each assigned to the tier that should produce it, by the same difficulty→tier logic: one entry per build unit carrying `id, build_unit, derived_from_stage_id (nullable), pick, budget, handoff_schema_ref, rationale`. Small mechanical function → cheapest tier; bounded well-specified feature → mid tier; tricky/high-blast-radius or cross-cutting logic → top tier; large long-horizon refactor → the largest-context/longest-horizon tier available. Each unit carries an explicit token/thinking **budget** and a **handoff_schema_ref** so it both self-truncates and knows exactly what shape to hand the next unit. Leave the array empty only if no build units are decided yet (e.g. a dry run against a bare intent).
...
- **Cost** — per-token list pricing for the models used (freshness-tagged), an order-of-magnitude estimate of spend if token counts are known, and — if the consuming system has a realized-spend telemetry source — the *actual* spend, named explicitly as such rather than inferred from list price alone. Prefer a leading-indicator telemetry source over a lagged billing export where one exists; name the gap explicitly if no such source is wired. A stage that ran on a non-invoiced tier (an open-weight model on hardware the operator owns) reports its amortized basis and the rework it caused, never $0, and says whether a hosted check overturned its output.
