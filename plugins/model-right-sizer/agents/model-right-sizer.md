...
4. **`blueprint_rows[]` budget + handoff + status** — the rest of each row, stated as deltas against any known defaults: `default_model` (if one exists, else `null`) and `keep_or_override`; `budget` — an explicit token/thinking ceiling (the BudgetThinker layer — e.g. `"<=8k thinking"`), not a vibe; `handoff_schema_ref` — the id (into `message_schemas[]`) of the structured payload this stage hands its consumer, or `"route_via_query_layer"` where flagged, or `"none"` for a terminal stage; and a one-clause `rationale`.
