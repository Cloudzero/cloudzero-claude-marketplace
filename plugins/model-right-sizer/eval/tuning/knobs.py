...
KNOBS = {
    ...
    "dispatch_floor_awareness": {
        "description": (
            ...
            "(5) -- levels 4 and 5 were both tried against a real held-out "
            "task's actuals (see "
            "`results/2026-08-22-pass7-blind-vs-chief-of-staff-actuals.md`) "
            "and both REJECTED. Level 4 (single-draw comparison, later "
            "shown to be within the noise floor): improved the one unit it "
            "was diagnosed from but regressed `accuracy_rate` overall, "
            "plausibly because naming exactly two qualifying shapes read as "
            "an implicit boundary rather than an illustration. Level 5 "
            "(proper 3-draw-averaged comparison, NOT a noise artifact): a "
            "general, non-enumerated '1.3-1.9x multiplier' instruction "
            "still lost on every single unit against level 3's own 3-draw "
            "average (`accuracy_rate` 0.167 -> 0.000, `mean_loss` 0.246 -> "
            "0.487) -- plausibly because naming an explicit multiplier "
            "invited a smaller base estimate to apply it to, netting lower "
            "than a holistic floor-plus-real-work judgment with no stated "
            "multiplier. IMPORTANT: level 3's OWN true accuracy, once "
            "properly 3-draw-averaged, is 0.167, not the 0.333 a single "
            "lucky draw originally reported -- single-draw dry-run "
            "comparisons on this task carry noise comparable in size to the "
            "wording effects being measured; trust only multi-draw-averaged "
            "results from this point forward. Both rejected levels are kept "
            "in the registry as tried-and-rejected data points, same "
            "discipline `calibration_decay`'s own history keeps -- do not "
            "re-promote level 4 or 5 to current-best without new evidence. "
        ...
        "levels": {
            ...
            5: (
                ...
                "up — and every one of those is a real tool call the "
                "ceiling must price in, not a free afterthought bolted "
                "onto an editor's turn. **This is a general calibration "
                "correction, not a checklist of qualifying examples** — "
                "averaged, multi-draw real-dispatch history across nearly "
                "every unit shape tested so far (a schema edit, a library "
                "module, an agent-instruction section, a skill runbook, a "
                "test suite) has landed 30–90% above a bare-content "
                "estimate, not just one or two named categories. Treat that "
                "as the norm for any sonnet-tier `low-tool-turn` or "
                "`agentic` unit doing real editing-plus-validation work, "
                "not the exception it would be if only a couple of shapes "
                "qualified: after estimating the apparent content size, "
                "multiply by roughly 1.3–1.9× before finalizing "
                "`token_ceiling`, leaning toward the higher end of that "
                "range for a unit touching a shared/cross-referenced file "
                "or reconciling its output against several other "
                "artifacts, and toward the lower end for a small, isolated, "
                "single-file addition with nothing else to check it "
                "against; `handoff_schema_ref`"
            ),
        },
    },
    "effort_tax": {
