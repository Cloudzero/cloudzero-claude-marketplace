...
HOLDOUT_TASKS = {
    ...
    "chief-of-staff-budget-enforcement": {
        ...
        "note": (
            "Second held-out task. Used three times now (pass 7's three "
            "blind dry-runs at dispatch_floor_awareness=2, =3, and =4, see "
            "results/2026-08-22-pass7-blind-vs-chief-of-staff-actuals.md) -- "
            "all three reads were genuinely blind (calibration ledger "
            "access withheld throughout), so no calibration-masking "
            "contamination happened, but this task's own n=6 has now been "
            "used to select a knob level three times (one accepted, one "
            "rejected), which is its own overfitting risk if pushed "
            "further -- iteration 3 specifically showed single-draw "
            "sampling noise on this task is roughly the same magnitude as "
            "the wording effects being measured, so further single-draw "
            "iterations against this same task are unlikely to reliably "
            "distinguish signal from noise without averaging multiple "
            "draws per candidate first. Has six real {actual_tokens, budgeted_tokens} "
