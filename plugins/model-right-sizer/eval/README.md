...
| [`check_citations.py`](check_citations.py) | Standalone script: cross-checks every claim in the ledger against `agents/model-right-sizer.md` (literal substring presence) and recomputes every claim with a derivable arithmetic relationship (via the two modules above), failing loudly on drift. |
...
1. Add a `papers[]` entry to `citation_ledger.json` with every numeric claim
   or formula it's cited for, quoting the source exactly.
