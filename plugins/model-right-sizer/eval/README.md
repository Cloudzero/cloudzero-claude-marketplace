...
| [`local_tier.py`](local_tier.py) | The lineup's one non-invoiced row, priced: amortized device cost plus power over measured throughput, the expected-rework term on top of it, and the throughput at which a local tier break-evens against a hosted one. Not a paper's formulas: these are the agent's own, stated as exact prose in `agents/model-right-sizer.md` and held to the same standard as `reasoning_budget.py`'s wall-clock gate, including tests that fail if the figures in that prose drift from what the code computes. |
